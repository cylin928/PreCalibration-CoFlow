...
class DivDM(Base):
    ...
    def make_dm(self, Q, current_date, agents):
        ...
        for ag in agt_list:
            ...
            V_acc = V_acc + Vavg*ag_par["Lr_c"]*5
            # Bound by Max and Min
            
            records[ag]["V_acc"].append(V_acc)
        ...
        for i, ag in enumerate(agt_list):
            ...
            div_req_ref = ag_par["a"] * x**2 + ag_par["b"] * x + ag_par["c"]
            mu = V_acc + div_req_ref
            if x <= 315 * 0.001:  # 315 mm is the threshold for drought year
                mu = mu * ag_par["ProratedRatio"]
